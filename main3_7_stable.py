# main3_4_admin_inspector_singlefile_mobile_pub.py
# ImageShare 3.4 — 管理员面板与巡查系统重构（单文件，移动端模板 + 公网支持）
# 功能：
#  - 完整管理员面板（图片审查 / 用户管理 / 巡查报告 / 版权申请占位 / 站点统计）
#  - 巡查员系统（申请 / 审核 / 巡查模式 / 报告审核 / 积分 / 冻结）
#  - 异步相似检测（上传后后台运行）
#  - 登录锁定（连续失败 5 次，锁定 5 分钟）
#  - 点赞/收藏/评论/头像/签名 等交互功能
#
...
APP_VERSION = "3.4"
SECRET_KEY = "please-change-this-secret-to-a-strong-value"
...
def ensure_data_and_migrate():
    """
    Ensure data.json exists and migrate/normalize structure to version APP_VERSION.
    This is idempotent and preserves existing content where possible.
    """
    ...
    # normalize inspector requests/reports/logs lists exist already
...
def background_similarity_check(image_id, filepath):
    """
    After upload we compute hash/phash and compare to existing images.
    Run in background to avoid blocking upload response.
    If similar images found, mark uploaded image as hidden and notify uploader/admin.
    """
    time.sleep(0.5)  # small delay to let save complete
    d = load_data()
...
<head>
  ...
  <style>
    ...
    @media (max-width: 576px) {
      ...
      button.btn { font-size: 0.85rem; padding: 0.25rem 0.5rem; }
...
<nav class="navbar navbar-expand-lg navbar-light fixed-top shadow-sm" style="background: var(--bg,#f5f7fb);">
  <div class="container-fluid">
    <a class="navbar-brand" href="{{ url_for('index') }}" style="color: var(--fg,#1b2430);">ImageShare 3.4</a>
...
# ---------------- UPLOAD (with background similarity check) ----------------
...
def detail(image_id):
    ...
    for c in im.get("comments", []):
        ...
        del_btn = ""
        if session.get("username") and session["username"] == by:
            del_btn = f'<button class="btn btn-sm btn-outline-danger" onclick="deleteComment({im.get("comments")[0].get("index") if im.get("comments") else 0})">删除</button>'
        comments_html += f'''
          <div class="mb-2">
...
# ---------------- INSPECTOR APPLY / MODE / REPORT SUBMIT ----------------
...
def inspector_apply():
    ...
    '''
    if not ok:
        body = f"<div class='alert alert-warning'>您暂不符合申请条件：{reason}</div>" + body
    return render(body)

def can_apply_inspector(username):
    d = load_data()
    u = d["users"].get(username)
    if not u: return False, "用户不存在"
    try:
        ...
        days = 0
    if days < 7:
        return False, f"账号注册时间不足 7 天（当前 {days} 天）"
    uploads = [im for im in d["images"].values() if im.get("uploader") == username]
    if len(uploads) < 5:
        return False, f"上传图片数量不足 5 张（当前 {len(uploads)} 张）"
    if u.get("banned_until"):
        if u.get("banned_until") == "permanent": return False, "存在永久封禁记录"
        try:
            t = datetime.datetime.fromisoformat(u.get("banned_until"))
            if datetime.datetime.utcnow() < t: return False, "账号当前处于封禁中，无法申请"
        except:
            return False, "账号存在封禁记录"
    # check false report rate
    reports_by_user = [r for r in d.get("reports", []) if r.get("by") == username]
    total_r = len(reports_by_user)
    false_count = 0
    for r in reports_by_user:
        iid = r.get("image_id"); im = d["images"].get(iid)
        if im:
            if im.get("status") == "visible" and not im.get("immune", False):
                false_count += 1
    if total_r > 0 and (false_count / total_r) > 0.2:
        return False, f"历史举报失误率过高（{false_count}/{total_r}）"
    return True, ""
...
def inspector_mode():
    ...
    # show images and allow submitting inspector reports
...
# ---------------- ADMIN: INSPECTOR REQUESTS & REPORTS ----------------
...
def admin_inspector_requests():
    ...
    if not reqs:
        ...
        for r in reqs:
            body += f'''
              <div class="card mb-2 p-2"><div><strong>{r.get("user")}</strong> 申请时间：{r.get("time")}</div>
...
def admin_set_inspection_result(action, report_index):
    ...
    # find actual index in inspector_reports list
...
def admin():
    ...
    body = '<div class="row">'
    # left column: controls / content depending on tab
    ...
    if tab == "overview":
        ...
        for uname,u in d["users"].items():
            ...
            body += f'<div class="d-flex justify-content-between align-items-center mb-2"><div><strong>{uname}</strong><br><small class="text-muted">{u.get("display_name")}</small><br><small class="text-muted">score={u.get("inspector_score",0)} points={u.get("points",100)}</small></div><div>'
        ...
        reqs = d.get("inspector_requests", [])
        if not reqs:
            body += '<p class="text-muted">暂无</p>'
        else:
            for r in reqs:
                body += f'<div class="card mb-2 p-2"><strong>{r.get("user")}</strong> 申请时间：{r.get("time")}<div>理由：{r.get("reason")}</div><div><a class="btn btn-sm btn-success" href="{url_for("admin_handle_inspector_apply", action="approve", user=r.get("user"))}">批准</a> <a class="btn btn-sm btn-danger" href="{url_for("admin_handle_inspector_apply", action="reject", user=r.get("user"))}">拒绝</a></div></div>'
    elif tab == "inspector_reports":
        return redirect(url_for("admin_inspector_reports"))
    elif tab == "copyright":
        body += '<h4>版权申请（占位）</h4><p class="text-muted">版权申请系统将在下一版本完善。</p>'
    else:
...
@app.route("/admin/action/<action>/<image_id>")
@admin_required
def admin_action(action, image_id):
    d = load_data()
    im = d["images"].get(image_id)
    if not im:
        flash("图片不存在", "err"); return redirect(url_for("admin"))
    if action == "approve":
        im["status"] = "visible"
        im["reports"] = []
        im["hidden"] = False
        save_data(d)
        flash("已设为可见", "ok"); return redirect(url_for("admin", tab="images"))
    if action == "ban_image":
        im["status"] = "banned"
        im["ban_time"] = datetime.datetime.utcnow().isoformat()
        save_data(d)
        push_notification(im.get("uploader"), "system", f"您的图片《{im.get('title')}》因管理员操作被封禁。")
        flash("图片已封禁", "ok"); return redirect(url_for("admin", tab="images"))
